...
public class WebSiteStats {
    ...
    public void process() throws IOException {
        ...
        new Scanner(new URL(_url).openStream(), "UTF-8").useDelimiter("\\s+|[,=\"<>.();{}:'\\-/?&|]")
                .forEachRemaining((token) -> {
                    _collectors.forEach((c) -> c.collect(token));
                    _mapC.forEach((mc) -> mc.collect(token, 1));
                    Hasher hasher = Hashing.sha1().newHasher();
                    hasher.putBytes(token.getBytes());
                    _biMap.collect(token, hasher.hash().toString());
                });

        _collectors.forEach((c) -> System.out.println(c.toString()));
        System.out.println("Maps...");
        _mapC.forEach((mc) -> System.out.println(mc.toString()));
